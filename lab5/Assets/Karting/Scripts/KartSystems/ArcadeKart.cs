...
namespace KartGame.KartSystems
{
    public class ArcadeKart : MonoBehaviour
    {
        ...
        void FixedUpdate()
        {
            ...
            // gather inputs
            float accel = PlayerInput.y;
            float turn = PlayerInput.x;
        ...
        void MoveVehicle(float accelInput, float turnInput)
        {
            ...
            {
                boostSystem.Play();
                maxSpeed = accelDirectionIsFwd ? finalStats.BoostSpeed : finalStats.ReverseSpeed;
            }
            else
            {
                boostSystem.Stop();
            ...
            // apply smoke particle systems
            if(Mathf.Abs(turningPower) > 3)
            {
                if (smokeLeft != null) smokeLeft.Play();
                if (smokeRight != null) smokeRight.Play();
            }
            else if (!boostSystem.isPlaying)
            {
                smokeLeft.Stop();
                smokeRight.Stop();
            }
        ...
        bool IsStuck()
        {
            ...
            if(GroundPercent <= 0 && speed < 0.01f && Mathf.Abs(PlayerInput.y) > 0)
                return true;
        ...
        IEnumerator StopBoostParticles(float time) {
